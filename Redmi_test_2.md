...
Обратился заведующий статистическим отделом больницы. На руках у него имеется распечатанный из системы годовой отчет. С комментарием "ну вы же понимаете, что это число явно рассчитано неверно?" показывает строку "умерло в процентном соотношении от поступивших", где указано значение 150%. Так как показатель такого рода не может быть более 100%, то явно содержится ошибка и следует поставить задачу на исправление.

**Была поставлена следующая задача:**
Инициатор: заведующий статистическим отделом. В отчете "Годовой отчет", сформированном за период 2019 года по отделению 1 и отделению 2 (фотография прилагается во вложении) в строке "умерло в процентном соотношении от поступивших" система рассчитала значение 150%. Данный показатель не может соответствовать действительным значениям, т.к. максимальное значение может быть равным 100%. Требуется исправить указанную ошибку в отчете.

**Задание:**
Опишите ошибки, которые были допущены в постановке.

**Ответ:**  В данной постановке задачи не было указано требуется ли оставлять данный расчет автоматическим или должна быть возможность внесения изменений вручную. Также нет информации будет ли проводиться контрольная проверка рассчитанных данных.


## Ситуация 10:
Обратился врач фельдшерско-акушерского пункта. Отличие ФАП от больницы чаще всего в том, что работает в нем 1-2 человека, а в данном конкретном случае - 1. Специально для этого врача была разработана система, куда врач заносит:
- ФИО пациента
- дату рождения
- адрес, дату осмотра
- диагноз 
- рекомендации по лечению. 
Врач заполняет все эти поля в системе и при сохранении у него распечатывается осмотр следующего вида.

Просьба врача состоит в том, что ему не хватает поля "Комментарий" для того, чтобы врач мог делать какие-то отметки на бланке осмотра, которые он не может по логике вписать в другие поля осмотра, например "Употреблял много сладкой пищи за последние 2 недели".

**Была поставлена следующая задача:**
Инициатор задачи: врач ФАП. Требуется доработать окно ввода информации у сотрудника ФАПа: добавить поле "Комментарий", тип - строчный, размерность - 2000 знаков.

**Задание:**
Опишите ошибки, которые были допущены в постановке

**Ответ:**  В данной постановке задачи не указано будет ли данное поле обязательным к заполнению или же его можно будет оставить пустым. Также нет информации о том требуется ли возможность изменения данных поля после сохранения.

## Ситуация 11:
Обратился сотрудник аптечного склада медикаментов, с просьбой поменять цену в приходном документе от 01.03.2020 в базе данных, так как накладная введена в систему, отчеты за данный период сданы, и поэтому через интерфейс системы поменять нельзя.

**Задание:**
Опишите ваши действия по решению вопроса пользователя.

**Ответ:**
1) Проверить достоверность данных по изменению.
2) Поменять цену через запрос в базе данных.
3) Задокументироавть факт измений и причину по которй они потребовались.
4) Позаботится о минимизации вероятности того, что ситуация повторится.
